...
```{r}
ss <- d %>% 
  group_by(Participant, trial_type, age_group, time_sec, measurement_type, Bin) %>% 
  summarise(ss_m = mean(value, na.rm = T))

ms <- ss %>% 
  group_by(trial_type, age_group, time_sec, measurement_type, Bin) %>% 
  summarise(n = n(),
...
```{r}
# make clusters for the bin vector; this feels a little hacky
...
Create list of data frames of permuted datasets.
...
( sum(nulls) / length(nulls) ) %>% round(3) # get the p-value rounded to 3 digits
```

Some key notes on this whole approach from Marvis and Oostenvald (2007):
